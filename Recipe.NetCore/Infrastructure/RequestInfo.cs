...
namespace Recipe.NetCore.Infrastructure
{
    public class RequestInfo<TDbContext> : IRequestInfo<TDbContext> where TDbContext : DbContext
    {
        ...
        public int? UserId => IsRequestContainHeaderItem(Constants.Strings.JwtClaimIdentifiers.Id) ? (int?)null : Convert.ToInt32(ApplicationContext.GetHttpContextHeaderItem(contextAccessor, Constants.Strings.JwtClaimIdentifiers.Id));
